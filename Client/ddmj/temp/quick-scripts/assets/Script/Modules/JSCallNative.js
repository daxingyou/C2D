...
cc._RF.push(module, '275fb5XmutGZIuaRvW0CTU8', 'JSCallNative', __filename);
...
function wxLogin() {
    if (ModuleManager_1.config.wxState !== 0) {
        ModuleManager_1.ui_manager.showTip('请先安装微信客户端，使用微信登录!');
...
function wxShare(url, title, des) {
    if (ModuleManager_1.config.wxState !== 0) {
        ModuleManager_1.ui_manager.showTip('请先安装微信客户端!');
        ModuleManager_1.ui_manager.hideLoading();
        return;
    }
    if (cc.sys.isNative && cc.sys.os === cc.sys.OS_ANDROID) {
...
function wxShareRecord(filePath) {
    if (ModuleManager_1.config.wxState !== 0) {
        ModuleManager_1.ui_manager.showTip('请先安装微信客户端，使用微信分享!');
        ModuleManager_1.ui_manager.hideLoading();
        return;
    }
    if (cc.sys.isNative && cc.sys.os === cc.sys.OS_ANDROID) {
