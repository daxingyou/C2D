...
export default class Club extends cc.Component {
    ...
    */
        ...
        if (dd.ui_manager.showLoading()) {
            ...
            dd.ws_manager.sendMsg(dd.protocol.CORPS_GET_CORPS_DETAILED, msg, (flag: number, content?: any) => {
                if (flag === 0) {//成功
                    ...
                    this.showClubTables(data.tables);
                } else if (flag === -1) {//超时
                } else {//失败,content是一个字符串
                    dd.ui_manager.showAlert(content, '温馨提示');
                }
     ...
     * @memberof Club
     */
    sendDeleteRecord(tableId: number) {
        if (dd.ui_manager.showLoading()) {
     ...
     * @memberof Club
     */
    showClubTables(clubtables: CorpsTableInner[]) {
        this.svNode_club_table.content.removeAllChildren();
        if (!clubtables) return;
        for (var i = 0; i < clubtables.length; i++) {
            let club_table_item = cc.instantiate(this.club_table_item_prefab);
            let table_script = club_table_item.getComponent('Club_Table_Item.ts');
            table_script.updateItem(clubtables[i], this._clubInfo, (clubTableData: CorpsTableInner) => {
