...
export default class MJCanvas extends cc.Component {
    ...
    */
     ...
     * @memberof MJCanvas
     */
    MJ_GamePush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
     ...
     * @memberof MJCanvas
     */
    MJ_SeatPush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
     ...
     * @memberof MJCanvas
     */
    MJ_DingQuePush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
     ...
     * @memberof MJCanvas
     */
    MJ_SwapCardPush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
     ...
     * @memberof MJCanvas
     */
    MJ_OutCardPush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
     ...
     * @memberof MJCanvas
     */
    MJ_BreakCardPush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
     ...
     * @memberof MJCanvas
     */
    MJ_DisbandPush = (event: cc.Event.EventCustom) => {
        let data = event.detail;
        dd.gm_manager.mjGameData = data as MJGameData;
        this.showMJInfo();
    };
    ...
    onLoad() {
        ...
        this.node.on("touchend", (event: cc.Event.EventTouch) => {
            ...
            if (dd.gm_manager.mjGameData && dd.gm_manager.mjGameData.tableBaseVo.gameState === MJ_Help.MJ_GameState.STATE_TABLE_SWAPCARD) {//如果是换牌
     ...
     * @memberof MJCanvas
     */
    sendOtherBreakCard(actType: MJ_Help.MJ_Act_Type, cardId: number, node_state?: cc.Node) {
        let obj = {
            'tableId': dd.gm_manager.mjGameData.tableBaseVo.tableId,
            'bt': actType,
            'cardId': JSON.stringify(cardId)
     ...
     * @memberof MJCanvas
     */
    private showLeftCard(type: number, cardId: number, parentNode: cc.Node, initCB: (mjCardNode: cc.Node) => void = null) {
        let mj_card_left: cc.Node = cc.instantiate(this.mj_card_left_prefab);
        mj_card_left.tag = cardId;
        let mc: MJ_Card = mj_card_left.getComponent('MJ_Card');
        if (cardId > 0) {
            let cardSF = this.getMJCardSF(cardId);
            mc.initData(cardId, cardSF);
        }
        let nodeFix = type === 1 ? cc.p(-1, 1) : cc.p(1, 1);
        let imgFix = type === 1 ? cc.p(-1, 1) : cc.p(1, 1);
     ...
     * @memberof MJCanvas
     */
    private showTableCard(type: number, cardId: number, parentNode: cc.Node, initCB: (mjCardNode: cc.Node) => void = null) {
        let mj_card_table: cc.Node = cc.instantiate(this.mj_card_table_prefab);
        mj_card_table.tag = cardId;
        let mc: MJ_Card = mj_card_table.getComponent('MJ_Card');
        if (cardId > 0) {
            let cardSF = this.getMJCardSF(cardId);
            mc.initData(cardId, cardSF);
        }
        let nodeFix = type === 0 ? cc.p(1, 1) : cc.p(1, 1);
        let imgFix = type === 0 ? cc.p(1, 1) : cc.p(-1, -1);
        mc.setFixCard(nodeFix, imgFix);
    ...
    */
    ...
    showChatInfo(data: ChatData) {
        ...
        if (data.type === 0) {
            let seatInfo = MJ_Help.getSeatById(data.accountId);
